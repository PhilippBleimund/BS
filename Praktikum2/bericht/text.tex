...
Worst-Fit sucht einen freien Speicherblock, welcher nach dem Allozieren den größten neuen Speicherblock erzeugt. Die Implementation \ref{listing:worstFit} ist gleich der Implementation von best-fit \ref{listing:bestFit} mit dem Unterschied, dass die Suchbedingung größere überbleibende Blöcke bevorzugt.

\section{Vergleich}

In \cref{fig:loglog} und \cref{fig:zoom} sind die Ergebnisse vom Test der Algorithmen dargestellt.

Im Vergleich der Algorithmen ist zu erkennen, dass first-fit, next-fit und best-fit ähnliche Ergebnisse liefern. Wohingegen next-fit etwas schlechter als first-fit und best-fit ist. Worst-fit ist der einzige Algorithmus, welcher schlechter abschneidet als die anderen.

In \cref{fig:best_algo} wurde zusätzlich zur maximalen Größe einer einzelnen Allozierung auch der verfügbare Speicher variiert. Es ist zu erkennen, dass best-fit in den meisten Fällen der beste Algorithmus ist. Best-fit ist in 11113 Kombinationen der beste Algorithmus. Am schlechtesten schneidet next-fit mit einem Wert von 684 ab. First-fit und worst-fit schneiden ähnlich mit 1659 und 993 Kombinationen etwas besser als next-fit ab. Die weiße Fläche stellt alle Kombinationen dar, in denen es mehr als zwei beste Algorithmen gibt und dominiert mit 25152 Kombinationen.
Die Transparenz gibt an, wie knapp ein Algorithmus gewonnen hat. Dadurch ist zu erkennen, dass bei einem Verhältnis von kleiner Allozierungsgröße und großem vorhandenen Speicher best-fit mit einem größeren Abstand der beste Algorithmus ist.
